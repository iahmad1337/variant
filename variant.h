...
public:
  ...
  constexpr variant() noexcept(std::is_nothrow_default_constructible_v<first_t>)
    requires(detail::default_ctor<first_t>)
  ...
  template <typename T>
    requires(detail::resolvable_conversion<T, Alternatives...> && !std::same_as<std::remove_cvref_t<T>, variant>)
  ...
  template <size_t I, class... Args>
    requires(I < sizeof...(Alternatives)) && std::is_constructible_v<meta::ith_t<I, Alternatives...>, Args...>
  ...
  constexpr variant& operator=(const variant& rhs)
    requires detail::trivial_copy_ctor<Alternatives...> && detail::trivial_copy_assign<Alternatives...> &&
                 detail::trivial_dtor<Alternatives...>
  = default;

  constexpr variant& operator=(const variant& rhs)
    requires detail::copy_ctor<Alternatives...> && detail::copy_assign<Alternatives...> &&
  ...
  constexpr variant& operator=(variant&& rhs)
    requires detail::trivial_move_ctor<Alternatives...> && detail::trivial_move_assign<Alternatives...> &&
                 detail::trivial_dtor<Alternatives...>
  = default;
  ...
  template <typename T>
    requires detail::resolvable_conversion<T, Alternatives...> &&
             (std::is_assignable_v<Alternatives&, std::remove_cvref_t<T>> || ...)
  constexpr variant&
  ...
  friend constexpr std::add_pointer_t<variant_alternative_t<I, variant<Alternatives...>>> get_if(
      variant<Alternatives...>* pv) noexcept {
    if (pv->index() == I) {
  ...
  template <size_t I>
    requires(I < sizeof...(Alternatives))
  friend constexpr std::add_pointer_t<const variant_alternative_t<I, variant<Alternatives...>>> get_if(
      variant<Alternatives...> const* pv) noexcept {
    if (pv->index() == I) {
      return recur::template get_if<I>(pv->alternatives);
    } else {
      return nullptr;
